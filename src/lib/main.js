...
function createEncodingFromCharset(name, charset, options = {}) {
  ...
  // base-x encode using long-division algorithm (byte-wise)
  function encode(bytes) {
    ...
    // produce string
    let out = '';
    for (let i = 0; i < zeros; i++) out += chars[0];
    for (let i = digits.length - 1; i >= 0; i--) out += chars[digits[i]];
    return out;
  }
  ...
  function decode(text) {
    ...
    for (let i = zeros; i < text.length; i++) {
      const value = chars.indexOf(text[i]);
      if (value === -1) throw new Error('invalid character in input');
      let carry = value;
      for (let j = 0; j < bytes.length; j++) {
        const v = bytes[j] * base + carry;
        bytes[j] = v & 0xff;
        carry = v >> 8;
      }
      while (carry > 0) {
...
// basE91 encode/decode implementation (variable 13/14-bit packing)
function basE91Encode(bytes) {
  ...
  for (let i = 0; i < bytes.length; i++) {
    ...
    if (n > 13) {
      ...
      out.push(alphabet[v % 91]);
      out.push(alphabet[Math.floor(v / 91)]);
    }
  }
  if (n > 0) {
    out.push(alphabet[b % 91]);
    if (n > 7 || b > 90) out.push(alphabet[Math.floor(b / 91)]);
  }
  return out.join('');
}
